...
class TelegramBot:
    ...
    def _setup_handlers(self):
        ...
        start_handler = MessageHandler(
            self.start, filters.command("start") & filters.private
        )
    ...
    async def handle_message(self, client: Client, message: Message):
        ...
        try:
            response = agent.run(
                f"telegram_id: {user_id} " + f"menssagem: {user_input}"
            )
        except Exception as e:
            self.logger.error(
                f"Erro ao processar a mensagem do usuário {user_id}: {e}", exc_info=True
            )
    ...
    async def handle_photo(self, client: Client, message: Message):
        ...
        try:
            ...
            self.logger.error(
                f"Erro ao processar a imagem do usuário {user_id}: {e}", exc_info=True
            )
